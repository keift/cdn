...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    await sleep(1000);
    let usable_link = await referral_links.find(async link => !await recently_redirected_links.find(async _link => _link.hash === link.acc_id ? await SHA256(link.url.split("http://").join("").split("https://").join("").split("/")[0] + "#" + link.acc_id + "@" + __ip_addr) : await SHA256(link.url.split("http://").join("//").split("https://").join("//") + "@" + __ip_addr) && _link.expiration_until >= Date.now()));
