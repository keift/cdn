...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    let { verified, referral_links, target_link_url } = JSON.parse(b64decode(token));
    if (verified === undefined || referral_links === undefined || target_link_url === undefined) throw new Error();
    ...
    startTimer(standby_time, async () => {
      ...
      await sleep(1000);
      location.href = (verified === true ? target_link_url : usable_link?.url || target_link_url).split("http://").join("//").split("https://").join("//");
    })
  } catch(err) {
    blinkTitle("Error Redirection Token");
    info.innerText = "Error Redirection Token";
