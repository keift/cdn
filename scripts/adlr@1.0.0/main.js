...
function blinkTitle(e) {
  (clearInterval(title_interval),
    ...
    (title_interval = setInterval(() => {
      ((document.title = e),
        (title_timeout = setTimeout(() => (document.title = "adlr"), 2e3)));
    }, 4e3)));
}
async function shortISGD(e) {
  let { data: t } = await axios.get("https://is.gd/create.php", {
    params: {
      url:
        "https://redr89e90cb6.tr.gg?token=" +
        b64encode(JSON.stringify({ url: e })),
      format: "json",
    },
  });
  return t.shorturl;
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (
      ...
      (blinkTitle("Error Redirection Token"),
        (e.innerText = "Error Redirection Token"));
      return;
    }
    try {
      ...
      for (let f = 0; f < r.accounts.length; f++) {
        let p;
        ("trlink" === r.accounts[f].name &&
          (p =
            "https://tr.link/full?api=" +
            r.accounts[f].token +
            "&url=" +
            b64encode(await shortISGD(d))),
          "ouoio" === r.accounts[f].name &&
            (p =
              "https://ouo.io/qs/" +
              r.accounts[f].token +
              "?s=" +
              encodeURIComponent(await shortISGD(d))),
          "uiiio" === r.accounts[f].name &&
            (p =
              "https://uii.io/full?api=" +
              r.accounts[f].token +
              "&url=" +
              b64encode(await shortISGD(d))),
          "exeio" === r.accounts[f].name &&
            (p =
              "https://exe.io/full?api=" +
              r.accounts[f].token +
              "&url=" +
              b64encode(await shortISGD(d))),
          p &&
